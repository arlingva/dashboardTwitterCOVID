...
![CLC,AMLO y CPA](images/3niveles.png =250x)
...
***
El promedio diario de cada usuario es:

- Andrés Manuel López Obrador:

```{r, echo = FALSE}
round(mean(df.tweet.date$AMLO.SMA, na.rm = TRUE), 2)
```

- Claudia Pavlovich Arellano

```{r, echo = FALSE}
round(mean(df.tweet.date$CPA.SMA, na.rm = TRUE), 2)
```

- Célida López Cárdenas

```{r, echo = FALSE}
round(mean(df.tweet.date$CLC.SMA, na.rm = TRUE), 2)
```
